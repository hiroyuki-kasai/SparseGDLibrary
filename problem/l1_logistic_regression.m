...
%
    ...
    function f = cost(w)
        ...
        f = -sum(log(sigmoid(y_train.*(w'*x_train))),2)/n_train + lambda * reg(w);
    ...
    function f = cost_batch(w, indices)
        ...
        f = -sum(log(sigmoid(y_train(indices).*(w'*x_train(:,indices))))/n_train,2) + lambda * reg(w);
    ...
    function w_opt = calc_solution(problem, maxiter, method)
        ...
        if strcmp(method, 'fista')
            [w_opt,~] = fista(problem, options);
        elseif strcmp(method, 'admm_lasso')
            [w_opt,~] = admm_lasso(problem, options);
        elseif strcmp(method, 'apg')
            options.step_init_alg = 'bb_init';
            options.step_alg = 'tfocs_backtracking'; 
            [w_opt,~] = apg(problem, options);
        else 
            options.step_init_alg = 'bb_init';
            options.step_alg = 'tfocs_backtracking'; 
            [w_opt,~] = apg(problem, options);            
